...
/*   Updated: 2023/06/16 10:16:31 by hucorrei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_pipe_and_execute(t_list *cur, t_prompt *p, int fds[2], int s_stdout)
{
	...
	while (cur)
	{
		...
		if (cur->next)
		{
			...
			if (cmd->outfile == 1)
				cmd->outfile = fds[1];
			if (((t_mini *)cur->next->content)->infile == 0)
				((t_mini *)cur->next->content)->infile = fds[0];
		}
		if (!ft_execute_builtin(cmd, p) && (g_status != 127 || cmd->full_path) \
			&& (g_status != 126 || cmd->full_path))
